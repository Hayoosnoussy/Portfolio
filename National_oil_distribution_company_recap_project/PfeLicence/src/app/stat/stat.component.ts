...
async getInvitations(){
  const promiseGet = await this.invitationService.getSent().toPromise()
  return promiseGet;
}
...
async getStatistique(){
  ...
  this.radarChartData  = [
    { data: [this.com-1,this.emp,this.sent, this.inv, , 0], label: 'Statistiques Global' }
  ];
}
...
public radarChartLabels: Label[] = ['Nombre des entreprises', 'Nombre des employee ', 'Nombre des invitations ','Nombre des inivtations envoyés'];
